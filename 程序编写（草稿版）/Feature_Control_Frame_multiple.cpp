//Feature_Control_Frame_multiple
...
void MyClass::do_it()
{
    ...
    try {
        ...
        for (int i = 0; i < numSelected; i++) {
            //强制转换选中对象为几何公差框
            objects1[0] = dynamic_cast<NXOpen::Annotations::DraftingFcf*>(
                theUI->SelectionManager()->GetSelectedTaggedObject(i));

            // 检查转换是否成功
            if (!objects1[0]) {//转换失败
                num_other++;
                sprintf(msg, "第%d个非形位公差框对象，跳过。", num_other);
               ...
               //提交修改
                ...
                sprintf(msg, "第%d个形位公差框对象，转换成功。", num_fcf);
                UF_UI_write_listing_window(msg);
                UF_UI_write_listing_window("\n");
        ...
        sprintf(msg, "执行完毕。共转换%d个形位公差框对象。", num_fcf);
        UF_UI_write_listing_window(msg);
        UF_UI_write_listing_window("\n");
    }
